...
def optimize_portfolio(risk_aversion, time_period):
    ...
    for _ in range(num_portfolios):
        ...
        results.append([portfolio_return, portfolio_stddev, portfolio_return/portfolio_stddev, utility])
        weights_record.append(weights)
    
    results_frame = pd.DataFrame(results, columns=['Return', 'Volatility', 'Sharpe Ratio', 'Utility'])
    ...
    plt.scatter(results_frame['Volatility'], results_frame['Return'], 
                c=results_frame['Sharpe Ratio'], cmap='YlGnBu', marker='o', alpha=0.3)
    plt.scatter(optimal_portfolio['Volatility'], optimal_portfolio['Return'],
                marker='*', color='r', s=500, label='Optimal Portfolio')
